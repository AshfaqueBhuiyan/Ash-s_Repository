...
public class Board {
	...
	public Board(Cell[][] grid, ArrayList<Block> blocks) {
		...
		for (int i = 0; i < blocks.size(); i++) { // creates new blocks and places them on the grid
...
// TODO
	...
	/**
	 * Helper method used to determine the index of the grabbed block within blocks
	 * array
	 *
	...
	private int currentblockIndex() {
		...
		for (int i = 0; i < blocks.size(); i++) {
			if (blocks.get(i).getFirstRow() == row && blocks.get(i).getFirstCol() == col) {
				blockNum = i;
			}
		}
		return blockNum;
	...
	public void moveGrabbedBlock(Direction dir) {
		...
		if (!over && grabbedBlock != null) {
			...
			if ((grabbedBlock.getOrientation() == Orientation.HORIZONTAL)) { // HORIZONTAL MOVE
				if (dir == RIGHT) { // Right possibility
					if (canPlaceBlock(row, col + grabbedBlock.getLength())) {
						...
						col = col + num;
						this.grid[row][originalCol + grabbedBlock.getLength()].setBlock(grabbedBlock);
						this.grid[row][originalCol].clearBlock();
						grabbedCell = this.grid[grabbedCell.getRow()][grabbedCell.getCol() + num];
					...
					if (canPlaceBlock(row, col + num)) {
						...
						blocks.get(grabbedBlockIndex).setFirstCol(originalCol + num);
						col = col + num;
						this.grid[row][originalCol + num].setBlock(grabbedBlock);
				...
				if (this.grid[row][col + grabbedBlock.getLength() - 1].isExit()) { // checks if game is over
					over = true;
				}
			} else if ((grabbedBlock.getOrientation() == Orientation.VERTICAL)) { // VERTICAL MOVE
				if (dir == DOWN) { // Up possibility
					if ((canPlaceBlock(row + grabbedBlock.getLength(), col))) {
						grabbedBlock.move(dir);
						row = row + num;
						blocks.get(grabbedBlockIndex).setFirstRow(originalRow + num);
						this.grid[originalRow + grabbedBlock.getLength()][col].setBlock(grabbedBlock);
						this.grid[originalRow][col].clearBlock();
						grabbedCell = this.grid[grabbedCell.getRow() + num][grabbedCell.getCol()];
						moveHistory.add(new Move(grabbedBlock, dir));
						moveCount++;
					}
					...
					if (canPlaceBlock(row + num, col)) {
						...
						row = row + num;
						blocks.get(grabbedBlockIndex).setFirstRow(originalRow + num);
						this.grid[originalRow + num][col].setBlock(grabbedBlock);
				...
				if (this.grid[row + grabbedBlock.getLength() - 1][col].isExit()) {
					over = true;
				}
			}
		}
	...
	public ArrayList<Move> getAllPossibleMoves() {
		...
		for (int i = 0; i < blocks.size(); i++) {
			if (blocks.get(i).getOrientation() == Orientation.HORIZONTAL) {
				if (canPlaceBlock(blocks.get(i).getFirstRow(), blocks.get(i).getFirstCol() + 2)) {
					possibleMoves.add(new Move(blocks.get(i), RIGHT));
				} else if (canPlaceBlock(blocks.get(i).getFirstRow(), blocks.get(i).getFirstCol() - 1)) {
			...
			if (blocks.get(i).getOrientation() == Orientation.VERTICAL) {
				...
				} else if (canPlaceBlock(blocks.get(i).getFirstRow() + 1, blocks.get(i).getFirstCol())) {
